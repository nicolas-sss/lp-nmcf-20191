...
df["suicides"].to_csv("./bases/suicide/visualizationSexYear/VisualizationSexYear.csv", index=False)

with open("./bases/suicide/visualizationSexYear/VisualizationSexYear.csv","r") as a:
    rdr= csv.reader( a )
    iterrdr = iter(rdr)
    maleSuicides=[]
    femaleSuicides=[]
    for i, row in enumerate(iterrdr):
        if i%2 == 0: #feminino 
              femaleSuicides.append(int(row[0])) 
        else: #masculino
                maleSuicides.append(int(row[0]))
